...
class Bitcoin_Voting_Shortcode_Voting
{
    ...
    {
        add_shortcode('bitcoin_voting', [$this, 'bitcoin_voting_render_shortcode_voting']);
    }
    ...
    function bitcoin_voting_render_shortcode_voting($atts)
    {
        ...
        $thank_you = get_post_meta($poll_id, '_bitcoin_voting_polls_thank_you_message', true);
        $description = get_post_meta($poll_id, '_bitcoin_voting_polls_description', true);
        $amount = get_post_meta($poll_id, '_bitcoin_voting_polls_amount', true);
        $start_date = get_post_meta($poll_id, '_bitcoin_voting_polls_starting_date', true);
        $end_date = get_post_meta($poll_id, '_bitcoin_voting_polls_ending_date', true);
        $num_options = 0;
        $options = array();
        $options_general = get_option('bitcoin_voting_options');
        $theme_class = $options_general['theme'] === 'dark' ? 'dark-theme' : 'light-theme';
        $active = get_post_meta($poll_id, '_bitcoin_voting_polls_active', true);
        $one_vote = get_post_meta($poll_id, '_bitcoin_voting_polls_one_vote', true);
        $collect_donor_info = get_post_meta($poll_id, '_bitcoin_voting_polls_collect_donor_info', true);
        $first_name = get_post_meta($poll_id, '_bitcoin_voting_polls_first_name_visibility')[0];
        $last_name = get_post_meta($poll_id, '_bitcoin_voting_polls_last_name_visibility')[0];
        $email = get_post_meta($poll_id, '_bitcoin_voting_polls_email_visibility')[0];
        $address = get_post_meta($poll_id, '_bitcoin_voting_polls_address_visibility')[0];
        $custom = get_post_meta($poll_id, '_bitcoin_voting_polls_custom_field_visibility')[0];
        // $custom_name = get_post_meta($poll_id, '_bitcoin_voting_polls_custom_field_name');
        $custom_name = 'Custom Field';
        error_log(print_r($first_name, true));
        // Check if poll is active
        if (!$active) {
            ob_start();
?>
            <div id="bitcoin-voting-form" class="bitcoin-voting-form  <?php echo esc_attr($theme_class); ?>" data-one-vote="<?php echo esc_attr($one_vote) ?>" data-donor-info="<?php echo esc_attr($collect_donor_info) ?> ">
        ...
        for ($i = 1; $i <= 4; $i++) {
            $option = get_post_meta($poll_id, "_bitcoin_voting_polls_option_{$i}", true);
            if (!empty($option)) {
                $options[$i] = $option;
                $num_options++;
            }
        ...
        ?>
            ...
            <div id="bitcoin-voting-blur-overlay" class="blur-overlay"></div>
            <?php
            $this->get_template('bitcoin-voting-modal', [
                'prefix' => 'bitcoin-voting-',
                'sufix' => '',
